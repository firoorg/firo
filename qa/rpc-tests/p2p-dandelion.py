...
class DandelionTest(BitcoinTestFramework):
    ...
    def setup_network(self):
        ...
        self.setup_nodes()
        print("Setting up network for dandelion 2.")
        # Tests 1,2,3: 0 --> 1 --> 2 --> 0
        connect_nodes(self.nodes[0],1)
        connect_nodes(self.nodes[1],2)
        connect_nodes(self.nodes[2],0)
        print("Setting up network for dandelion 3.")
        self.sync_all()
        print("Finished setting up network for dandelion.")

    def run_test(self):
        print("Running dandelion test 1")
        ...
        print("Running dandelion test 2")

        # Start networking thread
        network_thread_start()
        test_node0.wait_for_verack()

        print("Running dandelion test 3")

        # Get out of Initial Block Download (IBD)
        for node in self.nodes:
          node.generate(1)
        print("Running dandelion test 4")
        # Generate funds for node0
        node0.generate(101)
        print("Running dandelion test 5")
        ...
        test_3_passed = False
        tries_left = 5
        print("Running dandelion test 6")
