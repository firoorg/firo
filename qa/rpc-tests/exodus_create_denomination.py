...
class ExodusCreateDenominationTest(ExodusTestFramework):
    def run_test(self):
        super().run_test()
        ...
        self.nodes[0].exodus_sendissuancefixed(self.addrs[0], 1, 2, 0, '', '', 'Sigma Token', '', '', '1000000', 1)
        self.nodes[0].generate(1)
        ...
        assert_raises_message(
            ...
            self.nodes[0].exodus_sendcreatedenomination, self.addrs[0], -1, '1'
        )

        assert_raises_message(
            JSONRPCException,
            'Property identifier is out of range',
            self.nodes[0].exodus_sendcreatedenomination, self.addrs[0], 0, '1'
        )

        assert_raises_message(
            JSONRPCException,
            'Property identifier is out of range',
            self.nodes[0].exodus_sendcreatedenomination, self.addrs[0], 4294967296, '1'
        )
        ...
        assert_raises_message(
            ...
            self.nodes[0].exodus_sendcreatedenomination, self.addrs[0], 5, '1'
        )

        assert_raises_message(
            JSONRPCException,
            'Sender is not authorized to manage the property',
            self.nodes[0].exodus_sendcreatedenomination, self.addrs[1], 4, '1'
        )
        ...
        self.nodes[0].exodus_sendcreatedenomination(self.addrs[0], 4, '0.5')
        self.nodes[0].generate(1)
        self.nodes[0].exodus_sendcreatedenomination(self.addrs[0], 4, '1')
        self.nodes[0].generate(1)
        ...
        info = self.nodes[1].exodus_getproperty(4)
        ...
        for i in range(255 - 2):
            self.nodes[0].exodus_sendcreatedenomination(self.addrs[0], 4, str(i + 2))
        ...
        assert_raises_message(
            ...
            self.nodes[0].exodus_sendcreatedenomination, self.addrs[0], 4, '1000'
        )

if __name__ == '__main__':
    ExodusCreateDenominationTest().main()
