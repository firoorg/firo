...
extern "C" int LLVMFuzzerTestOneInput(uint8_t *buf, size_t len) {
    ...
    std::size_t n = fdp.ConsumeIntegralInRange<size_t>(2, 65535);
    std::size_t m = fdp.ConsumeIntegralInRange<size_t>(2, 65535);
    std::size_t N = (size_t) std::pow(n,m);

    GroupElement H;
    H.randomize();
    std::vector<GroupElement> Gi = fsp.GetRandomGroupVector(n*m);
    std::vector<GroupElement> Hi = fsp.GetRandomGroupVector(n*m);

    size_t commit_size = fdp.ConsumeIntegralInRange<size_t>(1, N);
    std::vector<GroupElement> S = fsp.GetRandomGroupVector(commit_size);
    std::vector<GroupElement> V = fsp.GetRandomGroupVector(commit_size);

    std::vector<uint8_t> indexes = fdp.ConsumeBytes<uint8_t>(N);
    std::vector<size_t> sizes;
    sizes.resize(N);
    for(size_t i=0; i < N; i++) {
        sizes[i] = fdp.ConsumeIntegralInRange<size_t>(0, N);
    }
    ...
    spark::Grootle grootle(H, Gi, Hi, n, m);
