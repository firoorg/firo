...
bool RPCConsole::RPCParseCommandLine(std::string &strResult, const std::string &strCommand, const bool fExecute, std::string * const pstrFilteredOut)
{
    ...
    for (chpos = 0; chpos < strCommandTerminated.size(); ++chpos)
    {
        ...
        switch(state)
        {
            case STATE_COMMAND_EXECUTED_INNER:     BOOST_FALLTHROUGH;
            ...
            case STATE_ARGUMENT:                    BOOST_FALLTHROUGH;// In or after argument 
            case STATE_EATING_SPACES_IN_ARG:        BOOST_FALLTHROUGH;
            case STATE_EATING_SPACES_IN_BRACKETS:   BOOST_FALLTHROUGH;
