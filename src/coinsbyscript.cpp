...
bool CCoinsViewByScriptDB::BatchWrite(CCoinsViewByScript* pcoinsViewByScriptIn, const uint256 &hashBlock) {
    ...
    LogPrintf("utxoindex", "Committing %zu coin address indexes to coin database...\n", (unsigned int)count);
...
void UpdateAddressIndex(const CBlock& block, CBlockUndo& blockundo, bool fConnect)
{
	...
	if (block.vtx.size() < 1)
	{
		throw std::runtime_error(std::string(__func__) + "UpdateAddressIndex failed - block.vtx.size() < 1");
	}
