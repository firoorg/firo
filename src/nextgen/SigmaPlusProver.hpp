...
void SigmaPlusProver<Exponent, GroupElement>::proof(
    ...
    Exponent x;
    NextGenPrimitives<Exponent, GroupElement>::get_x(proof_out.A_, proof_out.C_, proof_out.D_,x);
...
void SigmaPlusProver<Exponent, GroupElement>::sigma_commit(
        ...
        SigmaPlusProof<Exponent, GroupElement>& proof_out) {
    int N = commits.size();
    NextGenPrimitives<Exponent, GroupElement>::convert_to_sigma(l, n_, m_, sigma);
    for (int k = 0; k < m_; ++k) {
        Tk[k].randomize();
        Pk[k].randomize();
    }
    GroupElement B;
    NextGenPrimitives<Exponent, GroupElement>::commit(g_, h_, sigma, rB, B);
    ...
    //compute A
    GroupElement A;
    NextGenPrimitives<Exponent, GroupElement>::commit(g_, h_, a, rA, A);
    //compute C
    ...
    GroupElement C;
    NextGenPrimitives<Exponent, GroupElement>::commit(g_,h_, c, rC, C);
    //compute D
    ...
    GroupElement D;
    NextGenPrimitives<Exponent, GroupElement>::commit(g_,h_, d, rD, D);
    proof_out.A_ = A;
    proof_out.B_ = B;
    proof_out.D_ = D;
    proof_out.C_ = C;
    ...
    for (int i = 0; i < commits.size(); ++i) {
        ...
        std::vector<uint64_t> I = NextGenPrimitives<Exponent, GroupElement>::convert_to_nal(i, n_, m_);
        coefficients.push_back(sigma[I[0]]);
        coefficients.push_back(a[I[0]]);
        for (int j = 1; j < m_; ++j) {
            NextGenPrimitives<Exponent, GroupElement>::new_factor(sigma[j * n_ + I[j]], a[j * n_ + I[j]], coefficients);
        }
    ...
    for (int k = 0; k < m_; ++k) {
        ...
        proof_out.Qk.emplace_back(NextGenPrimitives<Exponent, GroupElement>::double_commit(g_, Exponent(uint64_t(0)), h_.get_g(0), Pk[k], h_.get_g(1), Tk[k]));
