...
//        return false;
    ...
    //check line 97
    GroupElement left = NextGenPrimitives<Exponent, GroupElement>::double_commit(g, (innerProductProof.c_ - delta), h1, proof.T_x1, h2, proof.T_x2);
    GroupElement V_z;
...
template<class Exponent, class GroupElement>
bool RangeVerifier<Exponent, GroupElement>::isValid(const RangeProof<Exponent, GroupElement>& proof)
{
    if(!(proof.A.isMember()
        && proof.S.isMember()
        && proof.T1.isMember()
