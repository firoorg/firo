...
UniValue exodus_createpayload_dexsell(const JSONRPCRequest& request)
{
    ...
    if (action <= CMPTransaction::UPDATE) { // actions 3 permit zero values, skip check
        amountForSale = ParseAmount(request.params[1], true); // TEXODUS/EXODUS is divisible
...
UniValue exodus_createpayload_enablefreezing(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
...
UniValue exodus_createpayload_disablefreezing(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
...
UniValue exodus_createpayload_freeze(const JSONRPCRequest& request)
{
...
UniValue exodus_createpayload_unfreeze(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw runtime_error(
