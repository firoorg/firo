...
CHDExodusMintTracker::CHDExodusMintTracker(std::string strWalletFile)
{
...
CHDExodusMintTracker::~CHDExodusMintTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CHDExodusMintTracker::Init()
{
...
bool CHDExodusMintTracker::Archive(CMintMeta& meta)
{
...
bool CHDExodusMintTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CHDMint dMint;
...
bool CHDExodusMintTracker::GetMetaFromSerial(const uint256 &hashSerial, CMintMeta& mMeta)
{
...
bool CHDExodusMintTracker::GetMetaFromPubcoin(const uint256& hashPubcoin, CMintMeta& mMeta)
{
...
std::vector<uint256> CHDExodusMintTracker::GetSerialHashes()
{
...
CAmount CHDExodusMintTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CHDExodusMintTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::list<CMintMeta> CHDExodusMintTracker::GetMints(bool fConfirmedOnly, bool fInactive) const
{
...
bool CHDExodusMintTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CHDExodusMintTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
...
bool CHDExodusMintTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CHDExodusMintTracker::UpdateState(const CMintMeta& meta)
{
    uint256 hashPubcoin = meta.GetPubCoinValueHash();
    CWalletDB walletdb(strWalletFile);
...
void CHDExodusMintTracker::Add(const CHDMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CHDExodusMintTracker::Add(const CSigmaEntry& zerocoin, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CHDExodusMintTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
    CMintMeta meta;
...
void CHDExodusMintTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    CMintMeta meta;
    if(!GetMetaFromPubcoin(hashPubcoin, meta))
        return;
...
void CHDExodusMintTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CHDExodusMintTracker::IsMempoolSpendOurs(const std::set<uint256>& setMempool, const uint256& hashSerial){
...
bool CHDExodusMintTracker::UpdateMetaStatus(const std::set<uint256>& setMempool, CMintMeta& mint, bool fSpend)
{
...
void CHDExodusMintTracker::UpdateFromBlock(const std::list<std::pair<uint256, MintPoolEntry>>& mintPoolEntries, const std::vector<CMintMeta>& updatedMeta){
...
void CHDExodusMintTracker::UpdateMintStateFromBlock(const std::vector<sigma::PublicCoin>& mints){
    CWalletDB walletdb(strWalletFile);
...
void CHDExodusMintTracker::UpdateSpendStateFromBlock(const sigma::spend_info_container& spentSerials){
    CWalletDB walletdb(strWalletFile);
    std::vector<CMintMeta> updatedMeta;
    std::list<std::pair<uint256, MintPoolEntry>> mintPoolEntries;
    uint160 hashSeedMasterEntry;
...
void CHDExodusMintTracker::UpdateMintStateFromMempool(const std::vector<GroupElement>& pubCoins){
    CWalletDB walletdb(strWalletFile);
    std::vector<CMintMeta> updatedMeta;
    std::list<std::pair<uint256, MintPoolEntry>> mintPoolEntries;
    uint160 hashSeedMasterEntry;
...
void CHDExodusMintTracker::UpdateSpendStateFromMempool(const vector<Scalar>& spentSerials){
    CWalletDB walletdb(strWalletFile);
    std::vector<CMintMeta> updatedMeta;
    std::list<std::pair<uint256, MintPoolEntry>> mintPoolEntries;
    uint160 hashSeedMasterEntry;
...
list<CSigmaEntry> CHDExodusMintTracker::MintsAsZerocoinEntries(bool fUnusedOnly, bool fMatureOnly){
...
std::vector<CMintMeta> CHDExodusMintTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fLoad, bool fWrongSeed)
{
...
std::set<uint256> CHDExodusMintTracker::GetMempoolTxids(){
...
void CHDExodusMintTracker::Clear()
{
    mapSerialHashes.clear();
}
