 ...
 */
bool Exodus_Encode_ClassB(const std::string& senderAddress, const CPubKey& redeemingPubKey,
 ...
 */
bool Exodus_Encode_ClassC(const std::vector<unsigned char>& vchPayload,
        std::vector<std::pair <CScript, int64_t> >& vecOutputs)
{
    std::vector<unsigned char> vchData;
    std::vector<unsigned char> vchExBytes = GetExMarker();
    vchData.insert(vchData.end(), vchExBytes.begin(), vchExBytes.end());
