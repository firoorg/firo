...
bool CalcCbTxMerkleRootQuorums(const CBlock& block, const CBlockIndex* pindexPrev, uint256& merkleRootRet, CValidationState& state)
{
    ...
    for (size_t i = 1; i < block.vtx.size(); i++) {
        ...
        if (tx->nVersion == 3 && tx->nType == TRANSACTION_QUORUM_COMMITMENT) {
            ...
            if (params.signingActiveQuorumCount >= 0 && v.size() == static_cast<unsigned long>(params.signingActiveQuorumCount)) {
                v.pop_back();
            }
            v.emplace_back(qcHash);
            hashCount++;
            assert(params.signingActiveQuorumCount < 0 || v.size() <= static_cast<unsigned long>(params.signingActiveQuorumCount));
        }
    }
