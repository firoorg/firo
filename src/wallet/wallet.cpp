...
bool CWallet::HasCollateralInputs(bool fOnlyConfirmed) const
{
    ...
    AvailableCoinsDash(vCoins, fOnlyConfirmed, NULL, false, ONLY_PRIVATESEND_COLLATERAL);
...
void CWallet::AvailableCoinsDash(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, bool fIncludeZeroValue, AvailableCoinsType nCoinType, bool fUseInstantSend) const
{
    vCoins.clear();
...
//                continue;
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = !(fZNode && pcoin->vout[i].nValue == 1000*COIN);
                } else if(nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && fZNode) found = pcoin->vout[i].nValue != 1000*COIN; // do not use Hot MN funds
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoinsDash(vCoins, true, coinControl, false, nPrivateSendRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT1000IFMN : ONLY_DENOMINATED);
...
//    sort(vCoins.rbegin(), vCoins.rend(), CompareByPriority());
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(fZNode && out.tx->vout[out.i].nValue == 1000*COIN) continue; //znode input
...
// znode
bool CWallet::GetZnodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    AvailableCoinsDash(vPossibleCoins, true, NULL, false, ONLY_1000);
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, std::vector<COutput>& vCoinsRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax)
{
    ...
    vector<COutput> vCoins;
    AvailableCoinsDash(vCoins, true, NULL, false, ONLY_DENOMINATED);
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        ...
        for (unsigned int i = 0; i < wtx.vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                if(fZNode && wtx.vout[i].nValue == 1000*COIN) continue;
