...
void CWallet::ReacceptWalletTransactions() {
    ...
    BOOST_FOREACH(PAIRTYPE(
    const int64_t, CWalletTx *)&item, mapSorted)
    {
        CWalletTx &wtx = *(item.second);
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                BOOST_FOREACH(
                const PAIRTYPE(const CWalletTx *, unsigned int) &
                coin, setCoins)
...
bool CWallet::CreateZerocoinMintTransaction(CScript pubCoin, int64_t nValue,
...
bool CWallet::InitLoadWallet() { std::string walletFile = GetArg("-wallet", DEFAULT_WALLET_DAT);
