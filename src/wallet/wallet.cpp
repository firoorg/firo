...
std::list<std::pair<spark::Coin, CSparkMintMeta>> CWallet::GetAvailableSparkCoins(const CCoinControl *coinControl) const {
    ...
    std::vector<CSparkMintMeta> vecMints = sparkWallet->ListSparkMints(true, true);
...
std::string CWallet::MintAndStoreSpark(
    ...
    std::list<CReserveKey> reservekeys;
    if (!CreateSparkMintTransactions(outputs, wtxAndFee, nFeeRequired, reservekeys, nChangePosRet, strError, coinControl, autoMintAll)) {
        return strError;
    }
