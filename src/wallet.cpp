...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            loop
            {
                ...
                bool fAllowFree = CTransaction::AllowFree(dPriority); // No free TXs in SMART
               //bool fAllowFree = false;					// No free TXs in SMART
                                ...
                                CWalletTx& wtxNew, CReserveKey& reservekey, CBigNum& coinSerial, uint256& txHash, CBigNum& zcSelectedValue, bool& zcSelectedIsUsed,  std::string& strFailReason)
{
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            if(!selectedPubcoin){
                strFailReason = _("you need to perform a Renew transactions and wait at least 6 minutes.");
                return false;
            }
