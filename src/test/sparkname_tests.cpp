...
BOOST_AUTO_TEST_CASE(reject_overflow_validity)
{
    Initialize(2700);

    std::string addr = GenerateSparkAddress();
    CMutableTransaction tx = CreateSparkNameTx("overflow", addr, std::numeric_limits<uint32_t>::max(), "", false, COIN);

    int oldHeight = chainActive.Height();
    GenerateBlock({tx});
    BOOST_CHECK_EQUAL(chainActive.Height(), oldHeight);
    BOOST_CHECK(!IsSparkNamePresent("overflow"));
}
