...
static ParallelOpThreadPool parallel_op_thread_pool;
...
GroupElement MultiExponent::get_multiple() {
    ...
    std::vector<GroupElement> thread_results;
    thread_results.resize(n_threads);
    ...
    for (int i = 0; i < n_threads; ++i) {
        ...
        parallel_tasks.emplace_back(parallel_op_thread_pool.PostTask([=, &thread_results] {
            thread_results[i] = get_multiple_single_thread(start_point, point_count);
        }));
        start_point += point_count;
    }

    for (std::future<void> &f: parallel_tasks)
        f.get();

    GroupElement r = thread_results[0];
    for (std::size_t n = 1; n < thread_results.size(); ++n)
        r += thread_results[n];
