...
CDarkSendRelay::CDarkSendRelay(CTxIn& vinSmartnodeIn, vector<unsigned char>& vchSigIn, int nBlockHeightIn, int nRelayTypeIn, CTxIn& in2, CTxOut& out2)
{
    vinSmartnode = vinSmartnodeIn;
    vchSig = vchSigIn;
...
std::string CDarkSendRelay::ToString()
{
    ...
    info << "vin: " << vinSmartnode.ToString() <<
...
{
    CSmartnode* pmn = mnodeman.GetSmartnodeByRank(nRank, nBlockHeight, MIN_PRIVATESEND_PEER_PROTO_VERSION);

    if(pmn != NULL){
        //printf("RelayThroughNode %s\n", pmn->addr.ToString().c_str());
        CNode* pnode = ConnectNode(CAddress(pmn->addr,nLocalServices), NULL);
        if(pnode) {
            //printf("Connected\n");
