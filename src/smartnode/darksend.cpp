...
void CDarksendPool::ProcessMessage(CNode *pfrom, std::string &strCommand, CDataStream &vRecv) {
    ...
    if (strCommand == NetMsgType::DSACCEPT) {
        ...
        if (!fSmartNode) {
            LogPrintf("DSACCEPT -- not a Smartnode!\n");
        ...
        CSmartnode *pmn = mnodeman.Find(activeSmartnode.vin);
        if (pmn == NULL) {
        ...
        CSmartnode *pmn = mnodeman.Find(dsq.vin);
        if (pmn == NULL) return;

        if (!dsq.CheckSignature(pmn->pubKeySmartnode)) {
        ...
        if (dsq.fReady) {
            if (!pSubmittedToSmartnode) return;
            if ((CNetAddr) pSubmittedToSmartnode->addr != (CNetAddr) pmn->addr) {
                LogPrintf("DSQUEUE -- message doesn't match current Smartnode: pSubmittedToSmartnode=%s, addr=%s\n", pSubmittedToSmartnode->addr.ToString(), pmn->addr.ToString());
                return;
            }

            if (nState == POOL_STATE_QUEUE) {
                LogPrint("privatesend", "DSQUEUE -- PrivateSend queue (%s) is ready on smartnode %s\n", dsq.ToString(), pmn->addr.ToString());
                SubmitDenominate();
            }
        } else {
            BOOST_FOREACH(CDarksendQueue
            q, vecDarksendQueue) {
                if (q.vin == dsq.vin) {
                    ...
                    LogPrint("privatesend", "DSQUEUE -- Smartnode %s is sending WAY too many dsq messages\n", pmn->addr.ToString());
                    return;
                }
            }
            ...
            if (pmn->nLastDsq != 0 && nThreshold > mnodeman.nDsqCount) {
                LogPrint("privatesend", "DSQUEUE -- Smartnode %s is sending too many dsq messages\n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
            ...
            LogPrint("privatesend", "DSQUEUE -- new PrivateSend queue (%s) from smartnode %s\n", dsq.ToString(), pmn->addr.ToString());
            if (pSubmittedToSmartnode && pSubmittedToSmartnode->vin.prevout == dsq.vin.prevout) {
                dsq.fTried = true;
            }
        ...
        if (!fSmartNode) {
            LogPrintf("DSVIN -- not a Smartnode!\n");
            PushStatus(pfrom, STATUS_REJECTED, ERR_NOT_A_MN);
            return;
        }
        ...
        if (fSmartNode) {
            // LogPrintf("DSSTATUSUPDATE -- Can't run on a Smartnode!\n");
            return;
        }

        if (!pSubmittedToSmartnode) return;
        if ((CNetAddr) pSubmittedToSmartnode->addr != (CNetAddr) pfrom->addr) {
            //LogPrintf("DSSTATUSUPDATE -- message doesn't match current Smartnode: pSubmittedToSmartnode %s addr %s\n", pSubmittedToSmartnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (!fSmartNode) {
            LogPrintf("DSSIGNFINALTX -- not a Smartnode!\n");
            return;
        }
        ...
        if (fSmartNode) {
            // LogPrintf("DSFINALTX -- Can't run on a Smartnode!\n");
            return;
        }

        if (!pSubmittedToSmartnode) return;
        if ((CNetAddr) pSubmittedToSmartnode->addr != (CNetAddr) pfrom->addr) {
            //LogPrintf("DSFINALTX -- message doesn't match current Smartnode: pSubmittedToSmartnode %s addr %s\n", pSubmittedToSmartnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (fSmartNode) {
            // LogPrintf("DSCOMPLETE -- Can't run on a Smartnode!\n");
            return;
        }

        if (!pSubmittedToSmartnode) return;
        if ((CNetAddr) pSubmittedToSmartnode->addr != (CNetAddr) pfrom->addr) {
            LogPrint("privatesend", "DSCOMPLETE -- message doesn't match current Smartnode: pSubmittedToSmartnode=%s  addr=%s\n", pSubmittedToSmartnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
...
std::string CDarksendPool::GetStatus() {
    ...
    if ((pCurrentBlockIndex && pCurrentBlockIndex->nHeight - nCachedLastSuccessBlock < nMinBlockSpacing) || !smartnodeSync.IsBlockchainSynced())
    ...
    switch (nState) {
        ...
        case POOL_STATE_ACCEPTING_ENTRIES:
            if (nEntriesCount == 0) {
                ...
                if (nStatusMessageProgress % 70 <= 40) return strprintf(_("Submitted following entries to smartnode: %u / %d"), nEntriesCount, GetMaxPoolTransactions());
                else if (nStatusMessageProgress % 70 <= 50) strSuffix = ".";
                else if (nStatusMessageProgress % 70 <= 60) strSuffix = "..";
                else if (nStatusMessageProgress % 70 <= 70) strSuffix = "...";
                return strprintf(_("Submitted to smartnode, waiting for more entries ( %u / %d ) %s"), nEntriesCount, GetMaxPoolTransactions(), strSuffix);
            }
        case POOL_STATE_SIGNING:
...
void CDarksendPool::CommitFinalTransaction() {
    if (!fSmartNode) return; // check and relay final tx only on smartnode
    ...
    if (!mapDarksendBroadcastTxes.count(hashTx)) {
        CDarksendBroadcastTx dstx(finalTransaction, activeSmartnode.vin, GetAdjustedTime());
        dstx.Sign();
...
void CDarksendPool::CheckTimeout() {
    ...
    if (!fEnablePrivateSend && !fSmartNode) return;
    ...
    int nLagTime = fSmartNode ? 0 : 10000; // if we're the client, give the server a few extra seconds before resetting.
...
void CDarksendPool::CheckForCompleteQueue() {
    ...
    if (nState == POOL_STATE_QUEUE && IsSessionReady()) {
        ...
        CDarksendQueue dsq(nSessionDenom, activeSmartnode.vin, GetTime(), true);
...
bool CDarksendPool::SendDenominate(const std::vector <CTxIn> &vecTxIn, const std::vector <CTxOut> &vecTxOut) {
    if (fSmartNode) {
        LogPrintf("CDarksendPool::SendDenominate -- PrivateSend from a Smartnode is not supported currently.\n");
        return false;
    }
    ...
    if (!nSessionID) {
        LogPrintf("CDarksendPool::SendDenominate -- No Smartnode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return false;
    }
...
bool CDarksendPool::CheckPoolStateUpdate(PoolState nStateNew, int nEntriesCountNew, PoolStatusUpdate nStatusUpdate, PoolMessage nMessageID, int nSessionIDNew) {
    ...
    strAutoDenomResult = _("Smartnode:") + " " + GetMessageByID(nMessageID);

    // if rejected at any state
    if (nStatusUpdate == STATUS_REJECTED) {
        LogPrintf("CDarksendPool::CheckPoolStateUpdate -- entry is rejected by Smartnode\n");
        UnlockCoins();
        SetNull();
        SetState(POOL_STATE_ERROR);
...
bool CDarksendPool::SignFinalTransaction(const CTransaction &finalTransactionNew, CNode *pnode) {
    if (fSmartNode || pnode == NULL) return false;
    ...
    LogPrintf("CDarksendPool::SignFinalTransaction -- pushing sigs to the smartnode, finalMutableTransaction=%s", finalMutableTransaction.ToString());
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun) {
    if (!fEnablePrivateSend || fSmartNode || !pCurrentBlockIndex) return false;
    ...
    if (mnodeman.size() == 0) {
        LogPrint("privatesend", "CDarksendPool::DoAutomaticDenominating -- No Smartnodes detected\n");
    ...
    LogPrint("privatesend", "Checking vecSmartnodesUsed: size: %d, threshold: %d\n", (int) vecSmartnodesUsed.size(), nThreshold_high);
    ...
    if ((int) vecSmartnodesUsed.size() > nThreshold_high) {
        ...
        LogPrint("privatesend", "  vecSmartnodesUsed: new size: %d, threshold: %d\n", (int) vecSmartnodesUsed.size(), nThreshold_high);
    }
    ...
    if (nLiquidityProvider || fUseQueue) {
        ...
        BOOST_FOREACH(CDarksendQueue & dsq, vecDarksendQueue)
        {
            ...
            CSmartnode *pmn = mnodeman.Find(dsq.vin);
            if (pmn == NULL) {
                LogPrintf("CDarksendPool::DoAutomaticDenominating -- dsq smartnode is not in smartnode list, smartnode=%s\n", dsq.vin.prevout.ToStringShort());
                continue;
            }
            ...
            vecSmartnodesUsed.push_back(dsq.vin);
            ...
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt to connect to smartnode from queue, addr=%s\n", pmn->addr.ToString());
            // connect to Smartnode and submit the queue request
            CNode *pnode = (pnodeFound && pnodeFound->fSmartnode) ? pnodeFound : ConnectNode(CAddress(pmn->addr, NODE_NETWORK), NULL, false, true);
            if (pnode) {
                pSubmittedToSmartnode = pmn;
                ...
                strAutoDenomResult = _("Error connecting to Smartnode.");
                continue;
            }
        }
    }
    ...
    while (nTries < 10) {
        CSmartnode *pmn = mnodeman.FindRandomNotInVec(vecSmartnodesUsed, MIN_PRIVATESEND_PEER_PROTO_VERSION);
        if (pmn == NULL) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Can't find random smartnode!\n");
            strAutoDenomResult = _("Can't find random Smartnode.");
            return false;
        }
        vecSmartnodesUsed.push_back(pmn->vin);

        if (pmn->nLastDsq != 0 && pmn->nLastDsq + nMnCountEnabled / 5 > mnodeman.nDsqCount) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Too early to mix on this smartnode!"
        ...
        LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt %d connection to Smartnode %s\n", nTries, pmn->addr.ToString());
        CNode *pnode = (pnodeFound && pnodeFound->fSmartnode) ? pnodeFound : ConnectNode(CAddress(pmn->addr, NODE_NETWORK), NULL, false, true);
        if (pnode) {
...
bool CDarksendPool::CreateNewSession(int nDenom, CTransaction txCollateral, PoolMessage &nMessageIDRet) {
    if (!fSmartNode || nSessionID != 0) return false;
    ...
    if (!fUnitTest) {
        ...
        CDarksendQueue dsq(nDenom, activeSmartnode.vin, GetTime(), false);
...
bool CDarksendPool::AddUserToExistingSession(int nDenom, CTransaction txCollateral, PoolMessage &nMessageIDRet) {
    if (!fSmartNode || nSessionID == 0 || IsSessionReady()) return false;
...
std::string CDarksendPool::GetMessageByID(PoolMessage nMessageID) {
    switch (nMessageID) {
        ...
        case ERR_NOT_A_MN:
            return _("This is not a Smartnode.");
        case ERR_QUEUE_FULL:
...
bool CDarksendQueue::Sign() {
    ...
    return CheckSignature(activeSmartnode.pubKeySmartnode);
}

bool CDarksendQueue::CheckSignature(const CPubKey &pubKeySmartnode) {
    std::string strMessage = vin.ToString() + boost::lexical_cast<std::string>(nDenom) + boost::lexical_cast<std::string>(nTime) + boost::lexical_cast<std::string>(fReady);
    std::string strError = "";

    if (!darkSendSigner.VerifyMessage(pubKeySmartnode, vchSig, strMessage, strError)) {
        LogPrintf("CDarksendQueue::CheckSignature -- Got bad Smartnode queue signature: %s; error: %s\n", ToString(), strError);
        return false;
    }
...
bool CDarksendBroadcastTx::CheckSignature(const CPubKey &pubKeySmartnode) {
    std::string strMessage = tx.GetHash().ToString() + boost::lexical_cast<std::string>(sigTime);
    std::string strError = "";
...
void CDarksendPool::RelayIn(const CDarkSendEntry &entry) {
    ...
    CNode *pnode = FindNode(pSubmittedToSmartnode->addr);
    if (pnode != NULL) {
...
void CDarksendPool::SetState(PoolState nStateNew) {
    if (fSmartNode && (nStateNew == POOL_STATE_ERROR || nStateNew == POOL_STATE_SUCCESS)) {
        LogPrint("privatesend", "CDarksendPool::SetState -- Can't set state to ERROR or SUCCESS as a Smartnode. \n");
        return;
    }
...
void CDarksendPool::UpdatedBlockTip(const CBlockIndex *pindex) {
    ...
    if (!fLiteMode && smartnodeSync.IsSmartnodeListSynced()) {
        NewBlock();
    }
}
...
void ThreadCheckDarkSendPool() {
    ...
    while (true) {
        ...
        if (smartnodeSync.IsBlockchainSynced() && !ShutdownRequested()) {
            ...
            if (nTick % 60 == 0) {
                mnodeman.ProcessSmartnodeConnections();
