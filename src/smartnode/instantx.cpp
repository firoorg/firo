...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate)
{
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        int n = mnodeman.GetSmartnodeRank(activeSmartnode.vin, nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

        if(n == -1) {
            LogPrint("instantsend", "CInstantSend::Vote -- Unknown Smartnode %s\n", activeSmartnode.vin.prevout.ToStringShort());
            ++itOutpointLock;
            continue;
        }
        ...
        if(itVoted != mapVotedOutpoints.end()) {
            BOOST_FOREACH(const uint256& hash, itVoted->second) {
                ...
                if(it2->second.HasSmartnodeVoted(itOutpointLock->first, activeSmartnode.vin.prevout)) {
        ...
        CTxLockVote vote(txHash, itOutpointLock->first, activeSmartnode.vin.prevout);
    ...
    if(it1 != mapVotedOutpoints.end()) {
        BOOST_FOREACH(const uint256& hash, it1->second) {
            if(hash != txHash) {
                ...
                std::map<uint256, CTxLockCandidate>::iterator it2 = mapTxLockCandidates.find(hash);
                if(it2->second.HasSmartnodeVoted(vote.GetOutpoint(), vote.GetSmartnodeOutpoint())) {
                    // yes, it did, refuse to accept a vote to include the same outpoint in another tx
                    // from the same smartnode.
                    // TODO: apply pose ban score to this smartnode?
        ...
        // we have votes by other smartnodes only (so far), let's continue and see who will win
...
void CInstantSend::CheckAndRemove()
{
    ...
    while(itOrphanVote != mapTxLockVotesOrphan.end()) {
        if(GetTime() - itOrphanVote->second.GetTimeCreated() > ORPHAN_VOTE_SECONDS) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan vote: txid=%s  smartnode=%s\n",
    ...
    // remove expired smartnode orphan votes (DOS protection)
    ...
    while(itSmartnodeOrphan != mapSmartnodeOrphanVotes.end()) {
        if(itSmartnodeOrphan->second < GetTime()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan smartnode vote: smartnode=%s\n",
...
bool CTxLockVote::IsValid(CNode* pnode) const
{
    ...
    int n = mnodeman.GetSmartnodeRank(CTxIn(outpointSmartnode), nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

    if(n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instantsend", "CTxLockVote::IsValid -- Outdated smartnode %s\n", outpointSmartnode.ToStringShort());
        return false;
    }
    LogPrint("instantsend", "CTxLockVote::IsValid -- Smartnode %s, rank=%d\n", outpointSmartnode.ToStringShort(), n);

    int nSignaturesTotal = COutPointLock::SIGNATURES_TOTAL;
    if(n > nSignaturesTotal) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Smartnode %s is not in the top %d (%d), vote hash=%s\n",
                outpointSmartnode.ToStringShort(), nSignaturesTotal, n, GetHash().ToString());
        return false;
    }
...
bool CTxLockVote::CheckSignature() const
{
    ...
    smartnode_info_t infoMn = mnodeman.GetSmartnodeInfo(CTxIn(outpointSmartnode));
    ...
    if(!darkSendSigner.VerifyMessage(infoMn.pubKeySmartnode, vchSmartnodeSignature, strMessage, strError)) {
...
bool CTxLockVote::Sign()
{
    ...
    if(!darkSendSigner.SignMessage(strMessage, vchSmartnodeSignature, activeSmartnode.keySmartnode)) {
        LogPrintf("CTxLockVote::Sign -- SignMessage() failed\n");
        return false;
    }

    if(!darkSendSigner.VerifyMessage(activeSmartnode.pubKeySmartnode, vchSmartnodeSignature, strMessage, strError)) {
