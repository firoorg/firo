...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit) {
	...
	ByteCodeExec exec(block, std::vector<ThemisTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block) {
	boost::filesystem::path themisDir = GetDataDir() / "vmExecLogs.json";
	std::stringstream ss;
	if (fIsVMlogFile) {
		ss << ",";
	}
	else {
		std::ofstream file(themisDir.string(), std::ios::out | std::ios::app);
		file << "{\"logs\":[]}";
		file.close();
	}
	...
	std::ofstream file(themisDir.string(), std::ios::in | std::ios::out);
...
bool ThemisTxConverter::extractionThemisTransactions(ExtractThemisTX& themistx) {
	std::vector<ThemisTransaction> resultTX;
	...
	themistx = std::make_pair(resultTX, resultETP);
	return true;
}

bool ThemisTxConverter::receiveStack(const CScript& scriptPubKey) {
...
bool ThemisTxConverter::parseEthTXParams(EthTransactionParams& params) {
	try {
...
ThemisTransaction ThemisTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut) {
	ThemisTransaction txEth;
	if (etp.receiveAddress == dev::Address() && opcode != OP_CALL) {
		txEth = ThemisTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
	}
	else {
		txEth = ThemisTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
	}
...
//    block.lastHeight = pindex->nHeight;
	...
	///////////////////////////////////////////////// // themis
	ThemisDGP themisDGP(globalState.get(), fGettingValuesDGP);
	globalSealEngine->setThemisSchedule(themisDGP.getGasSchedule(pindex->nHeight + 1));
...
//                                      !(pindexBIP34height->GetBlockHash() == chainparams.GetConsensus().BIP34Hash));
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
		...
		if (tx.HasCreateOrCall() && !hasOpSpend) {
			...
			ThemisTxConverter convert(tx, &view, &block.vtx);

			ExtractThemisTX resultConvertThemisTX;
			if (!convert.extractionThemisTransactions(resultConvertThemisTX)) {
			...
			if (!CheckMinGasPrice(resultConvertThemisTX.second, minGasPrice))
			...
			dev::u256 gasAllTxs = dev::u256(0);
			ByteCodeExec exec(block, resultConvertThemisTX.first, blockGasLimit);
			...
			for (ThemisTransaction& qtx : resultConvertThemisTX.first) {
			...
			{
				for (size_t k = 0; k < resultConvertThemisTX.first.size(); k++) {
					...
					tri.push_back(TransactionReceiptInfo{ block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertThemisTX.first[k].from(), resultConvertThemisTX.first[k].to(),
...
//    LogPrintf("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
		...
		dev::h256 oldHashStateRoot(globalState->rootHash()); // themis
		dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // themis
        ...
        if (!rv) {
			...
			globalState->setRoot(oldHashStateRoot); // themis
			globalState->setRootUTXO(oldHashUTXORoot); // themis
