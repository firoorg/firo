...
bool  SigmaPlusVerifier<Exponent, GroupElement>::verify(
    ...
    if(left != LelantusPrimitives<Exponent, GroupElement>::double_commit(g_, Exponent(uint64_t(0)), h_.get_g(0), proof.zV_, h_.get_g(1), proof.zR_))
        return false;
...
bool SigmaPlusVerifier<Exponent, GroupElement>::batchverify(
    ...
    for (int t = 0; t < M; ++t)
    {
       right += (LelantusPrimitives<Exponent, GroupElement>::double_commit(g_, Exponent(uint64_t(0)), h_.get_g(0), proofs[t].zV_, h_.get_g(1), proofs[t].zR_)) * y[t];
       Exponent e;
       for(int i = 0; i < N; ++i)
            e += f_i_[t][i];
       e *= serials[t] * y[t];
