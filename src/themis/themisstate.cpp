...
ThemisState::ThemisState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = ThemisState::openDB(_path + "/themisDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ThemisState::ThemisState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute ThemisState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, ThemisTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createThemisAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            themis::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> ThemisState::vins() const // temp
{
...
void ThemisState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* ThemisState::vin(dev::Address const& _a) const
{
    return const_cast<ThemisState*>(this)->vin(_a);
}

Vin* ThemisState::vin(dev::Address const& _addr)
{
...
// void ThemisState::commit(CommitBehaviour _commitBehaviour)
// {
...
void ThemisState::kill(dev::Address _addr)
{
...
void ThemisState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address ThemisState::createThemisAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void ThemisState::deleteAccounts(std::set<dev::Address>& addrs){
...
void ThemisState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void ThemisState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
