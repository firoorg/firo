...
bool ThemisDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule ThemisDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t ThemisDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t ThemisDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t ThemisDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t ThemisDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool ThemisDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void ThemisDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void ThemisDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void ThemisDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void ThemisDGP::createParamsInstance(){
...
dev::Address ThemisDGP::getAddressForBlock(unsigned int blockHeight){
...
void ThemisDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void ThemisDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void ThemisDGP::parseStorageOneUint64(uint64_t& value){
...
void ThemisDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule ThemisDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
